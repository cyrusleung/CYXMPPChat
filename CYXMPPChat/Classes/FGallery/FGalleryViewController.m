...
/*
@interface UINavigationBar(FGalleryViewController)
@end
@implementation UINavigationBar(FGalleryViewController)

-(void)drawRect:(CGRect)rect
{
    [[UIImage imageNamed:@"navigationbar_b"] drawInRect:rect];
}
@end
*/
...
- (void)loadView
{
    ...
    pagesLabel_custom=[[UILabel alloc] initWithFrame:CGRectMake(0, 0, 50.0, 20.0)];
    pagesLabel_custom.backgroundColor=[UIColor clearColor];
    pagesLabel_custom.textAlignment=UITextAlignmentRight;
    pagesLabel_custom.textColor=[UIColor whiteColor];
    pagesLabel_custom.font=[UIFont systemFontOfSize:14.0];
    ...
    //UIBarButtonItem *pagesBarItem = [[UIBarButtonItem alloc] initWithCustomView:pagesLabel_custom];
	
	// add prev next to front of the array
	[_barItems insertObject:_nextButton atIndex:0];
    //[_barItems insertObject:pagesBarItem atIndex:0];
    ...
    UIImageView *bgBottomImg=[[UIImageView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT-95.0, SCREEN_WIDTH, 75.0)];
    bgBottomImg.image=[UIImage imageNamed:@"images_5.png"];
    [self.view addSubview:bgBottomImg];
    
    pagesLabel_custom.frame=CGRectMake(SCREEN_WIDTH-55.0, SCREEN_HEIGHT-104.0, 50.0, 20.0);
    [self.view addSubview:pagesLabel_custom];
    
    imgTextLabel=[[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(pagesLabel_custom.frame), SCREEN_WIDTH, 20.0)];
    imgTextLabel.backgroundColor=[UIColor clearColor];
    imgTextLabel.textAlignment=UITextAlignmentCenter;
    imgTextLabel.textColor=[UIColor whiteColor];
    imgTextLabel.font=[UIFont systemFontOfSize:14.0];
    [self.view addSubview:imgTextLabel];
    
    backButton=[UIButton buttonWithType:UIButtonTypeCustom];
    backButton.frame=CGRectMake(0,  CGRectGetMaxY(imgTextLabel.frame), 44.0, 44.0);
    [backButton setBackgroundColor:[UIColor clearColor]];
    [backButton setImage:[UIImage imageNamed:@"images_1.png"] forState:UIControlStateNormal];
    ...
    [backButton addTarget:self action:@selector(backToViewAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:backButton];

    imgSaveButton=[UIButton buttonWithType:UIButtonTypeCustom];
    imgSaveButton.frame=CGRectMake(SCREEN_WIDTH-98.0, CGRectGetMaxY(imgTextLabel.frame), 44.0, 44.0);
    [imgSaveButton setBackgroundColor:[UIColor clearColor]];
    [imgSaveButton setImage:[UIImage imageNamed:@"images_2.png"] forState:UIControlStateNormal];
    [imgSaveButton.imageView setContentMode:UIViewContentModeScaleAspectFit];
    [imgSaveButton setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
    [imgSaveButton addTarget:self action:@selector(imgSaveButtonAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:imgSaveButton];
    
    imgShareButton=[UIButton buttonWithType:UIButtonTypeCustom];
    imgShareButton.frame=CGRectMake(SCREEN_WIDTH-49.0, CGRectGetMaxY(imgTextLabel.frame), 44.0, 44.0);
    [imgShareButton setBackgroundColor:[UIColor clearColor]];
    [imgShareButton setImage:[UIImage imageNamed:@"images_3.png"] forState:UIControlStateNormal];
    [imgShareButton.imageView setContentMode:UIViewContentModeScaleAspectFit];
    [imgShareButton setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
    [imgShareButton addTarget:self action:@selector(imgShareButtonAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:imgShareButton];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[UIBarButtonItem alloc] initWithTitle:@"全部" style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)];
...
-(void)imgSaveButtonAction
{
    FGalleryPhotoView * photoView = [_photoViews objectAtIndex:_currentIndex];
    
    UIImageWriteToSavedPhotosAlbum(photoView.imageView.image, nil, nil,nil);
    if (!HUD) {
...
-(void)imgShareButtonAction
{
    if ([_photoSource respondsToSelector:@selector(photoShareAction)]) {
        [self backToViewAction];
        [_photoSource performSelector:@selector(photoShareAction) withObject:nil afterDelay:0.5];
    }else{
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"暂无分享功能" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alert show];
    }
}
...
- (void)updateTitle
{
    if (!_hideTitle){
        ...
        pagesLabel_custom.text=[NSString stringWithFormat:@"%i %@ %i", _currentIndex+1, NSLocalizedString(@"/", @"") , [_photoSource numberOfPhotosForPhotoGallery:self]];
    }else{
        [self setTitle:@""];
    }
}
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [self arrangeThumbs];
    [self.navigationItem.rightBarButtonItem setTitle:@"大图"];
...
- (FGalleryPhoto*)createGalleryPhotoForIndex:(NSUInteger)index
{
	...
	}else if(sourceType==FGalleryPhotoSourceTypeLocalImage){
        UIImage *image_source=[_photoSource photoGallery:self  atIndex:index];
        thumbPath = @"";
		fullsizePath = @"";
		photo = [[FGalleryPhoto alloc] initWithThumbnailImage:image_source thumb:thumbPath fullsizePath:fullsizePath delegate:self];
    }else
	{
